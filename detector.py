...
#returns a new tensor which indexes the input with indices specified by the index varible which is a long tensor
im_dim_list = torch.index_select(im_dim_list, 0, output[:,0].long()) #input, axis, indices

scaling_factor = torch.min(inp_dim/im_dim_list,1)[0].view(-1,1) #slicing the indices of the torch.min output

output[:,[1,3]] -= (inp_dim - scaling_factor*im_dim_list[:,0].view(-1,1))/2 #coords conform to the dimensions of the img with padded area
output[:,[2,4]] -= (inp_dim - scaling_factor*im_dim_list[:,1].view(-1,1))/2

output[:,1:5] /= scaling_factor #scaling back up the coords of the bbox to the original img (divided in letterbox function)
